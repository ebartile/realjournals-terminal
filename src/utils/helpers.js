...
import { flattenDeep, isArray, isFunction, isString, keyBy, mapValues, map, mergeWith, reduce, ceil } from 'lodash';
...
export function isExternalLink(path) {
  // Check if the path starts with 'http://' or 'https://' (an absolute URL)
  if (path.startsWith('http://') || path.startsWith('https://')) {
    // Parse the URL to extract the domain
    const url = new URL(path);
    return url.hostname !== window.location.hostname;
  } else if (path.startsWith('/')) {
    // Check if the path is a relative path (starts with '/')
    return false; // It's considered an external link since it's not relative to the current domain
  } else {
    // Handle other cases (e.g., domain-relative URLs)
    // Parse the current window location's hostname and compare it with the provided path
    const currentDomain = window.location.hostname;
    const pathParts = path.split('/');
    return pathParts[0] !== currentDomain;
  }
}
...
export function formatUrl(url) {
  ...
  return url; // URL already has the prefix
}
