import React, { cloneElement, isValidElement, memo, useContext, useRef } from 'react';
...
function Item(props) {
  ...
  const renderLayout = (baseChildren, inputContext) => {
    if (!isEmpty(inputContext)) {
      baseChildren = <FormInputContext.Provider value={inputContext}>{baseChildren}</FormInputContext.Provider>;
    }
    return baseChildren;
  };
  ...
  return (
    <Field {...props} messageVariables={variables} validateTrigger={validateTrigger} trigger={trigger}>
...
const MemoInput = memo(
  ...
  (prev, next) => {
    return prev.value === next.value && prev.disabled === next.disabled && prev.update === next.update;
  }
);
